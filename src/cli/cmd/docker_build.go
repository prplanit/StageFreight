	...
	return pipeline.Phase{
		...
		Run: func(pc *pipeline.PipelineContext) (*pipeline.PhaseResult, error) {
			...
			for _, step := range plan.Steps {
				...
				if err != nil && step.Push && publish.IsHarborProjectMissingPushError(step.Registries, stderrBuf.String()) {
					diag.Info("harbor: push failed with project-not-found, attempting auto-create and retry")
					if ensureErr := publish.EnsureHarborProjects(pc.Ctx, step.Registries); ensureErr == nil {
						stderrBuf.Reset()
						stepResult, layers, err = bx.BuildWithLayers(pc.Ctx, step)
						if stepResult == nil {
							stepResult = &build.StepResult{Name: step.Name, Status: "failed"}
			...
			if len(remoteTags) > 0 {
				...
				if pc.Verbose {
					pushBx.Stderr = io.MultiWriter(os.Stderr, &pushStderrBuf)
				} else {
				...
				if err := pushBx.PushTags(pc.Ctx, remoteTags); err != nil {
					...
					if publish.IsHarborProjectMissingPushError(pushRegs, pushStderrBuf.String()) {
						diag.Info("harbor: push failed with project-not-found, attempting auto-create and retry")
						if ensureErr := publish.EnsureHarborProjects(pc.Ctx, pushRegs); ensureErr == nil {
							pushStderrBuf.Reset()
							if pc.Verbose {
								pushBx.Stderr = io.MultiWriter(os.Stderr, &pushStderrBuf)
							} else {
